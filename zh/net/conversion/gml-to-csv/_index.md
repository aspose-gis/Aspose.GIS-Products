---
title: 将 GML 转换为 CSV GIS 数据，使用 .NET 或 C#
weight: 120
url: /zh/net/conversion/gml-to-csv/ 
description: 使用我们的本地 API 将 GML 转换为 CSV 在 .NET Framework、.NET Core、C# 上。
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="将 GML 转换为 CSV 在 C#" h2="使用服务器端 API 将 GML 导出到 CSV，无需像 ArcGIS、ArcMap、QGIS 或 Google Earth 这样的软件。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="CSV" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="GML" >}}
...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 GML 转换为 CSV" %}}

 为了将 GML 转换为 CSV，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="在 C# 中将 GML 转换为 CSV 的步骤" %}}
...
2.  源驱动程序，让 API 了解输入格式。设置为 Drivers.Gml
3.  指向目标 CSV 数据文件的字符串类型文件路径
4.  目标驱动程序以指定输出格式。设置为 Drivers.Csv
...
{{% blocks/products/pf/agp/code-block title="将 GML 保存为 CSV - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="GML 到 CSV 在线演示" sectionDescription="访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/gml-to-csv) 立即将 GML 转换为 CSV。在线演示具有以下优势" >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 GML 文件，它将立即转换为 CSV。" >}}
...
扩展名为 CSV（逗号分隔值）的文件表示纯文本文件，其中包含以逗号分隔的值的数据记录。CSV 文件中的每一行都是文件中记录集的新记录。当打算从一个存储系统传输到另一个存储系统时，会生成此类文件。由于所有应用程序都可以识别由逗号分隔的记录，因此将这些数据文件导入数据库非常方便。
...
{{< blocks/products/pf/agp/other-supported-section title="其他支持的转换" subTitle="您还可以将 GML 转换为许多其他文件格式，包括下面列出的几种。" >}}
