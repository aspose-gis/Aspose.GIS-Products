...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 TOPOJSON 转换为 JSON" %}}

 为了将 TOPOJSON 转换为 JSON，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="将 TOPOJSON 转换为 JSON 在 C# 中的步骤" %}}

{{% blocks/products/pf/agp/text %}}

 可以通过使用
 *VectorLayer.Convert*
 方法，只需一行代码即可完成转换。它接受四个参数，如下详细所述。
...
JSON（JavaScript 对象表示法）是一种开放标准的文件格式，用于共享数据，它使用人类可读的文本来存储和传输数据。 JSON 文件存储在带有 .json 扩展名的文件中。 JSON 需要更少的格式设置，并且是 XML 的一个很好的替代方案。 JSON 源自 JavaScript，但是一种与语言无关的数据格式。许多现代编程语言都支持生成和解析 JSON。 application/json 是用于 JSON 的媒体类型。
了解更多
...
{{< blocks/products/pf/agp/other-supported-section title="其他支持的转换" subTitle="您还可以将 TOPOJSON 转换为许多其他文件格式，包括下面列出的一些。" >}}
...
{{< blocks/products/pf/agp/other-supported-section-item href="https://products.aspose.com/gis/net/conversion/topojson-to-shx" name="TOPOJSON TO SHX" description="Shapefile 文件扩展名" >}}
