...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 Shapefile 转换为 JSON" %}}

 为了将 Shapefile 转换为 JSON，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="在 C# 中将 Shapefile 转换为 JSON 的步骤" %}}
...
{{% blocks/products/pf/agp/code-block title="将 Shapefile 保存为 JSON - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="Shapefile to JSON 在线演示" sectionDescription="访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/shapefile-to-json) 立即将 Shapefile 转换为 JSON。在线演示具有以下优势" >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 Shapefile 文件，它将立即转换为 JSON。" >}}
...
SHP 是用于表示 ESRI Shapefile 的主要文件类型之一的文件扩展名。它以矢量数据的形式表示地理空间信息，供地理信息系统 (GIS) 应用程序使用。该格式作为开放规范开发，以便促进 ESRI 与其他软件产品之间的互操作性。

        {{< /blocks/products/pf/agp/about-file-text >}}

        {{< blocks/products/pf/agp/about-file-text fileFormat="JSON" readMoreLink="https://docs.fileformat.com/gis/geojson/" >}}

基于 JSON 的格式，旨在表示地理特征及其非空间属性。此格式定义了不同的 JSON（JavaScript 对象表示法）对象及其连接方式。JSON 代表有关地理特征、其空间范围和属性的集体信息。
