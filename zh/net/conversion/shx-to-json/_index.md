...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 SHX 转换为 JSON" %}}

 为了将 SHX 转换为 JSON，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="将 SHX 转换为 JSON 在 C# 中的步骤" %}}
...
{{% blocks/products/pf/agp/code-block title="将 SHX 保存为 JSON - C#" offSpacer="" %}}

```cs
// 将 SHX GIS 数据转换为 JSON 数据
...
{{< blocks/products/pf/agp/demobox sectionTitle="SHX 到 JSON 在线演示" sectionDescription="通过访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/shx-to-json) 立即将 SHX 转换为 JSON。在线演示具有以下优势" >}}
        {{< blocks/products/pf/agp/democard icon="fa-cogs" text=" 无需下载 Aspose GIS API。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-edit" text=" 仅选择 JSON 作为输出格式，然后。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 SHX 文件，它将立即转换为 JSON。" >}}
...
形状索引格式，用于特征几何图形的位置索引，以便快速向前和向后查找
了解更多
...
JSON（JavaScript 对象表示法）是一种开放标准的文件格式，用于共享数据，它使用人类可读的文本来存储和传输数据。 JSON 文件存储在 .json 扩展名中。 JSON 需要更少的格式化，并且是 XML 的一个很好的替代方案。 JSON 源自 JavaScript，但是一种与语言无关的数据格式。许多现代编程语言都支持生成和解析 JSON。 application/json 是用于 JSON 的媒体类型。
了解更多
