...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 GDB 转换为 CSV" %}}

 为了将 GDB 转换为 CSV，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="在 C# 中将 GDB 转换为 CSV 的步骤" %}}
...
{{% blocks/products/pf/agp/code-block title="将 GDB 保存为 CSV - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="GDB 到 CSV 在线演示" sectionDescription="访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/gdb-to-csv) 立即将 GDB 转换为 CSV。在线演示具有以下优势" >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 GDB 文件，它将立即转换为 CSV。" >}}
...
GDB 是 Esri ArcGIS 平台使用的格式。此文件夹包含一个集合，其中包含 .gdbtable、.gdbtablx、.gdbindexes、.atx、.spx 和 .freelist 文件等。

        {{< /blocks/products/pf/agp/about-file-text >}}

        {{< blocks/products/pf/agp/about-file-text fileFormat="CSV" readMoreLink="https://docs.fileformat.com/spreadsheet/csv/" >}}

具有 CSV（逗号分隔值）扩展名的文件表示纯文本文件，其中包含以逗号分隔的值的数据记录。CSV 文件中的每一行都是文件中记录集中的新记录。当打算从一个存储系统传输到另一个存储系统时，会生成此类文件。由于所有应用程序都可以识别由逗号分隔的记录，因此将这些数据文件导入数据库非常方便。
...
{{< blocks/products/pf/agp/other-supported-section title="其他支持的转换" subTitle="您还可以将 GDB 转换为许多其他文件格式，包括下面列出的几种。" >}}
...
{{< blocks/products/pf/agp/other-supported-section-item href="https://products.aspose.com/gis/net/conversion/gdb-to-kml" name="GDB TO KML" description="关键字标记语言" >}}
