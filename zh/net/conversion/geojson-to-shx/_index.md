...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 GEOJSON 转换为 SHX" %}}

 为了将 GEOJSON 转换为 SHX，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="将 GEOJSON 转换为 SHX 在 C# 中的步骤" %}}
...
{{% blocks/products/pf/agp/code-block title="将 GEOJSON 保存为 SHX - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="GEOJSON 到 SHX 在线演示" sectionDescription="通过访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/geojson-to-shx) 立即将 GEOJSON 转换为 SHX。在线演示具有以下优势" >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 GEOJSON 文件，它将立即转换为 SHX。" >}}
...
GeoJSON 是一种基于 JSON 的格式，旨在表示地理要素及其非空间属性。此格式定义了不同的 JSON（JavaScript 对象表示法）对象以及它们的连接方式。JSON 格式表示有关地理特征、其空间范围和属性的集体信息。该文件的对象可以指示几何体（点、线串、多边形）、要素或要素集合。这些要素反映地址和地点为点，街道、主干道和边界为线串，国家/地区、省份和土地区域为多边形。使用 GeoJSON，不同的移动路由和导航应用程序可以指示其服务的覆盖范围。TopoJSON 是 GeoJSON 的扩展，它体积更小并编码了地理空间拓扑。
了解更多
...
形状索引格式，用于对要素几何体进行定位的格式索引，以便能够快速向前和向后查找
了解更多
