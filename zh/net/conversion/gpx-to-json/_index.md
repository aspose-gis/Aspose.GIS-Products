...
{{% blocks/products/pf/agp/content h2="如何使用 C# 将 GPX 转换为 JSON" %}}

 为了将 GPX 转换为 JSON，我们将使用
...
{{% blocks/products/pf/agp/feature-section-col title="将 GPX 转换为 JSON 在 C# 中的步骤" %}}
...
{{% blocks/products/pf/agp/code-block title="将 GPX 保存为 JSON - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="GPX to JSON 在线演示" sectionDescription="访问我们的 [在线演示网站](https://products.aspose.app/gis/conversion/gpx-to-json) 将 GPX 转换为 JSON。在线演示具有以下优势" >}}
        {{< blocks/products/pf/agp/democard icon="fa-cogs" text=" 无需下载 Aspose GIS API。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-edit" text=" 仅选择 JSON 作为输出格式。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" 上传您的 GPX 文件，它将立即转换为 JSON。" >}}
...
GPX 扩展名代表 GPS Exchange 格式，用于在应用程序和互联网上的 Web 服务之间交换 GPS 数据。它是一种轻量级的 XML 格式，包含 GPS 数据，即航点、路线和轨迹，以便被多个程序导入和读取。 GPX 是开放的，并受到各种应用程序和 GPS 设备的支持。

        {{< /blocks/products/pf/agp/about-file-text >}}

        {{< blocks/products/pf/agp/about-file-text fileFormat="JSON" readMoreLink="https://docs.fileformat.com/gis/geojson/" >}}

基于 JSON 的格式，旨在表示地理要素及其非空间属性。此格式定义了不同的 JSON（JavaScript 对象表示法）对象以及它们的连接方式。JSON 表示有关地理特征、其空间范围和属性的集体信息。
