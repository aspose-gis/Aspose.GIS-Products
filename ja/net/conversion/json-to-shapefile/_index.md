---
title: .NET または C# を使用して JSON を Shapefile GIS データに変換する
weight: 120
url: /ja/net/conversion/json-to-shapefile/ 
description: 当社のオンプレミス API を使用して、.NET Framework、.NET Core、C# で JSON を Shapefile に変換します。
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C# で JSON を Shapefile に変換する" h2="ArcGIS、ArcMap、QGIS、または Google Earth などのソフトウェアを必要とせずに、サーバー側の API を使用して JSON を Shapefile にエクスポートします。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="Shapefile" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="JSON" >}}
...
{{% blocks/products/pf/agp/content h2="C# を使用して JSON を Shapefile に変換する方法" %}}

JSON を Shapefile に変換するには、
...
{{% blocks/products/pf/agp/feature-section-col title="C# で JSON を Shapefile に変換する手順" %}}
...
2.  入力形式に関する API に通知するためのソースドライバー。GeoJson に設定します。
...
{{% blocks/products/pf/agp/code-block title="JSON を Shapefile として保存する - C#" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="JSON to Shapefile ライブデモ" sectionDescription="[Live Demos ウェブサイト](https://products.aspose.app/gis/conversion/json-to-shapefile) にアクセスして、今すぐ JSON を Shapefile に変換してください。ライブデモには次の利点があります。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-cogs" text="Aspose GIS API のダウンロードは不要です。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-edit" text="Shapefile を出力形式として選択するだけです。" >}}
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text="JSON ファイルをアップロードすると、すぐに Shapefile に変換されます。" >}}
...
SHP は、ESRI Shapefile の主要なファイルタイプの 1 つの拡張子です。ベクトルデータ形式で地理空間情報を表し、地理情報システム (GIS) アプリケーションで使用されます。この形式は、ESRI と他のソフトウェア製品間の相互運用性を促進するためにオープン仕様として開発されました。
...
{{< blocks/products/pf/agp/other-supported-section title="その他のサポートされている変換" subTitle="JSON を、以下を含む他の多くのファイル形式にも変換できます。" >}}
