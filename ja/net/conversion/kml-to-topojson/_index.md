---
title: .NET または C# を使用して KML を TopoJSON GIS データに変換する
weight: 120
url: /ja/net/conversion/kml-to-topojson/ 
description: 当社のオンプレミス API を使用して、.NET Framework、.NET Core、C# で KML を TopoJSON に変換します。
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C# で KML を TopoJSON に変換する" h2="ArcGIS、ArcMap、QGIS、または Google Earth などのソフトウェアを必要とせずに、サーバーサイド API を使用して KML を TopoJSON にエクスポートします。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="TopoJSON" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="KML" >}}
...
{{% blocks/products/pf/agp/content h2="C# を使用して KML を TopoJSON に変換する方法" %}}

KML を TopoJSON に変換するには、
...
{{% blocks/products/pf/agp/feature-section-col title="C# で KML を TopoJSON に変換する手順" %}}

{{% blocks/products/pf/agp/text %}}

*VectorLayer.Convert* 
メソッドを使用すると、コード行だけで変換できます。以下に示すように、4 つのパラメーターを受け取ります。
...
4.  出力形式を指定する宛先ドライバー。TopoJson に設定します。
...
{{% blocks/products/pf/agp/code-block title="C# - KML を TopoJSON として保存" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="KML to TopoJSON のライブデモ" sectionDescription="[ライブデモ](https://products.aspose.app/gis/conversion/kml-to-topojson) Web サイトにアクセスして、今すぐ KML を TopoJSON に変換してください。ライブデモには以下の利点があります。" >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text=" KML ファイルをアップロードすると、すぐに TopoJSON に変換されます。" >}}
...
TopoJSON は、トポロジをエンコードする GeoJSON の拡張です。地理的特徴とその非空間属性を表すように設計された JSON ベースの形式です。この形式は、さまざまな JSON (JavaScript Object Notation) オブジェクトとそれらの結合方法を定義します。JSON 形式は、地理的特徴、その空間範囲、およびプロパティに関する集合的な情報を示します。
...
{{< blocks/products/pf/agp/other-supported-section title="その他のサポートされている変換" subTitle="KML を、以下を含む他の多くのファイル形式に変換することもできます。" >}}
