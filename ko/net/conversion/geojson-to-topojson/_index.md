---
title: .NET을 이용한 GEOJSON에서 TOPOJSON GIS 데이터로 변환
weight: 150
url: /ko/net/conversion/geojson-to-topojson/
description: .NET Framework, .NET Core에서 GIS 데이터 변환을 위한 On-Premise API를 사용해 보세요.
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C#으로 GEOJSON을 TOPOJSON으로 변환" h2="ArcGIS, ArcMap, QGIS 또는 Google Earth와 같은 소프트웨어가 필요 없이 서버 측 API를 사용하여 GEOJSON을 TOPOJSON으로 내보내기." logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="TOPOJSON" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="GEOJSON" >}}
...
{{% blocks/products/pf/agp/content h2="C#을 사용하여 GEOJSON을 TOPOJSON으로 변환하는 방법" %}}

GEOJSON을 TOPOJSON으로 변환하려면 [Aspose.GIS for .NET](https://products.aspose.com/gis/net) API를 사용합니다. 이 API는 C# 플랫폼용 기능이 풍부하고 강력하며 사용하기 쉬운 GIS API입니다. 
...
{{% blocks/products/pf/agp/feature-section-col title="C#에서 GEOJSON을 TOPOJSON으로 변환하는 단계" %}}
...
{{% blocks/products/pf/agp/code-block title="C# - GEOJSON을 TOPOJSON으로 저장" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="GEOJSON을 TOPOJSON으로 변환하는 라이브 데모" sectionDescription="[라이브 데모 웹사이트](https://products.aspose.app/gis/conversion/geojson-to-topojson)를 방문하여 GEOJSON을 즉시 TOPOJSON으로 변환하세요. 라이브 데모에는 다음과 같은 이점이 있습니다." >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text="GEOJSON 파일을 업로드하면 즉시 TOPOJSON으로 변환됩니다." >}}
...
GeoJSON은 지리적 특징과 비 공간 속성을 나타내도록 설계된 JSON 기반 형식입니다. 이 형식은 다양한 JSON (JavaScript Object Notation) 객체와 결합 방식을 정의합니다. JSON 형식은 지리적 특징, 공간 범위 및 속성에 대한 집합 정보를 나타냅니다. 이 파일의 개체는 점, 선 문자열 및 다각형으로 주소와 장소를 나타내고 주요 도로와 경계는 선 문자열로, 국가, 지방 및 토지 지역은 다각형으로 나타낼 수 있습니다. GeoJSON을 사용하면 다양한 모바일 라우팅 및 탐색 애플리케이션이 서비스 범위를 표시할 수 있습니다. TopoJSON은 크기가 더 작고 지리 공간 위상 구조를 인코딩하는 GeoJSON의 확장입니다.
자세히 보기
...
TopoJSON은 위상 구조를 인코딩하는 GeoJSON의 확장입니다. TopoJSON 파일에서 기하 도형은 개별적으로 표현되는 것이 아니라 공유 선 세그먼트라고 하는 아크로 연결됩니다.
자세히 보기
...
{{< blocks/products/pf/agp/other-supported-section title="지원되는 다른 변환" subTitle="다음과 같이 GEOJSON을 많은 다른 파일 형식으로도 변환할 수 있습니다." >}}
