---
title: .NET 또는 C#을 사용하여 OSM을 CSV GIS 데이터로 변환
weight: 120
url: /ko/net/conversion/osm-to-csv/ 
description: .NET Framework, .NET Core, C#에서 On-Premise API를 사용하여 OSM을 CSV로 변환해 보세요.
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C#에서 OSM을 CSV로 변환" h2="ArcGIS, ArcMap, QGIS 또는 Google Earth와 같은 소프트웨어가 필요 없이 서버 측 API를 사용하여 OSM을 CSV로 내보내기" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="CSV" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="OSM" >}}
...
{{% blocks/products/pf/agp/content h2="C#을 사용하여 OSM을 CSV로 변환하는 방법" %}}

OSM을 CSV로 변환하려면 [Aspose.GIS for .NET](https://products.aspose.com/gis/net) 
...
{{% blocks/products/pf/agp/feature-section-col title="C#에서 OSM을 CSV로 변환하는 단계" %}}
...
1. 소스 OSM 파일에 대한 문자열 형식의 파일 경로
2. 입력 형식을 API에 알리기 위한 소스 드라이버입니다. Drivers.OsmXml로 설정합니다.
3. 대상 CSV 데이터 파일에 대한 문자열 형식의 파일 경로
...
{{% blocks/products/pf/agp/code-block title="C# - OSM을 CSV로 저장" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="OSM을 CSV로 변환하는 실시간 데모" sectionDescription="[실시간 데모 웹사이트](https://products.aspose.app/gis/conversion/osm-to-csv)를 방문하여 OSM을 즉시 CSV로 변환해 보세요. 실시간 데모에는 다음과 같은 이점이 있습니다." >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text="OSM 파일을 업로드하면 즉시 CSV로 변환됩니다." >}}
...
확장자가 CSV(Comma Separated Values)인 파일은 쉼표로 구분된 값을 포함하는 레코드의 데이터가 있는 일반 텍스트 파일입니다. CSV 파일의 각 줄은 파일에 포함된 레코드 세트에서 새 레코드입니다. 이러한 파일은 데이터를 한 저장 시스템에서 다른 저장 시스템으로 전송할 때 생성됩니다. 모든 응용 프로그램이 쉼표로 구분된 레코드를 인식하기 때문에 데이터베이스로 가져오는 것이 매우 편리합니다.
...
{{< blocks/products/pf/agp/other-supported-section title="지원되는 다른 변환" subTitle="다음과 같이 OSM을 다른 많은 파일 형식으로도 변환할 수 있습니다." >}}
