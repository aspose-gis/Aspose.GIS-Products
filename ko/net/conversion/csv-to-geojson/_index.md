---
title: .NET 또는 C#을(를) 사용하여 CSV를 GeoJSON GIS 데이터로 변환
weight: 120
url: /ko/net/conversion/csv-to-geojson/ 
description: .NET Framework, .NET Core, C#에서 On-Premise API를 사용하여 CSV를 GeoJSON으로 변환해 보세요.
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C#에서 CSV를 GeoJSON으로 변환" h2="ArcGIS, ArcMap, QGIS 또는 Google Earth와 같은 소프트웨어가 필요 없이 서버 측 API를 사용하여 CSV를 GeoJSON으로 내보내기" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="GeoJSON" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="CSV" >}}
...
{{% blocks/products/pf/agp/content h2="C#을(를) 사용하여 CSV를 GeoJSON으로 변환하는 방법" %}}

CSV를 GeoJSON으로 변환하려면 C# 플랫폼용 강력하고 사용하기 쉬운 GIS API인 [Aspose.GIS for .NET](https://products.aspose.com/gis/net) 
API를 사용합니다. 
...
{{% blocks/products/pf/agp/feature-section-col title="C#에서 CSV를 GeoJSON으로 변환하는 단계" %}}

{{% blocks/products/pf/agp/text %}}

*VectorLayer.Convert* 
메서드를 사용하여 변환을 수행할 수 있습니다. 자세한 내용은 아래와 같습니다. 4개의 매개변수를 사용합니다.

{{% /blocks/products/pf/agp/text %}}

1. 소스 CSV 파일의 문자열 형식 파일 경로
...
저희 API는 모든 주요 플랫폼 및 운영 체제를 지원합니다. 아래 코드를 실행하기 전에 시스템에 다음 전제 조건이 있는지 확인하십시오.

{{% /blocks/products/pf/agp/text %}}

{{% /blocks/products/pf/agp/feature-section-col %}}

{{% blocks/products/pf/agp/code-block title="C# - CSV를 GeoJSON으로 저장" offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="CSV를 GeoJSON으로 변환하는 라이브 데모" sectionDescription="[라이브 데모 웹사이트](https://products.aspose.app/gis/conversion/csv-to-geojson)에서 CSV를 GeoJSON으로 즉시 변환해 보세요. 라이브 데모에는 다음과 같은 이점이 있습니다." >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text="CSV 파일을 업로드하면 즉시 GeoJSON으로 변환됩니다." >}}
...
CSV(Comma Separated Values) 확장자를 가진 파일은 쉼표로 구분된 값을 사용하여 데이터 레코드를 포함하는 일반 텍스트 파일입니다. CSV 파일의 각 줄은 파일에 포함된 레코드 세트에서 새 레코드입니다. 이러한 파일은 데이터를 한 저장 시스템에서 다른 저장 시스템으로 전송할 때 생성됩니다. 모든 응용 프로그램이 쉼표로 구분된 레코드를 인식하므로 데이터베이스로 가져오는 것이 매우 편리합니다.

        {{< /blocks/products/pf/agp/about-file-text >}}

        {{< blocks/products/pf/agp/about-file-text fileFormat="GeoJSON" readMoreLink="https://docs.fileformat.com/gis/geojson/" >}}

GeoJSON은 지리적 특징과 비 공간 속성을 나타내기 위해 설계된 JSON 기반 형식입니다. 이 형식은 다양한 JSON(JavaScript Object Notation) 개체와 결합 방식에 대해 정의합니다. JSON은 지리적 특징, 공간 범위 및 속성에 대한 집합 정보를 나타냅니다.
