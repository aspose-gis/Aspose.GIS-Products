---
title: SHX를 GEOJSON GIS 데이터로 변환하기 .NET 
weight: 240
url: /ko/net/conversion/shx-to-geojson/ 
description: .NET Framework, .NET Core에서 GIS 데이터 변환을 위한 On-Premise API를 사용해 보세요.
---

{{< blocks/products/pf/main-wrap-class isAutogenPage="true">}}
{{< blocks/products/pf/upper-banner h1="C#으로 SHX를 GEOJSON으로 변환" h2="ArcGIS, ArcMap, QGIS 또는 Google Earth와 같은 소프트웨어가 필요 없이 서버 측 API를 사용하여 SHX를 GEOJSON으로 내보내세요." logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/gis/aspose_gis-for-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="GEOJSON" pfName="Aspose.GIS" subTitlepfName="for .NET" downloadUrl="" fileiconsmall1="PNG" fileiconsmall2="JPG" fileiconsmall3="BMP" fileiconsmall4="TIFF" fileiconsmall5="SHX" >}}
...
{{% blocks/products/pf/agp/content h2="C#을 사용하여 SHX를 GEOJSON으로 변환하는 방법" %}}

SHX를 GEOJSON으로 변환하려면 다음을 사용합니다.
...
패키지 관리자를 열고 다음을 검색합니다.
**Aspose.GIS** 
설치하세요. 패키지 관리자 콘솔에서 다음 명령어를 사용할 수도 있습니다.
...
{{% blocks/products/pf/agp/feature-section-col title="C#에서 SHX를 GEOJSON으로 변환하는 단계" %}}

{{% blocks/products/pf/agp/text %}}

*VectorLayer.Convert* 
메서드를 사용하여 변환을 수행할 수 있습니다. 아래에 자세히 설명된 4개의 매개변수를 사용합니다.

{{% /blocks/products/pf/agp/text %}}

1. 소스 SHX 파일의 문자열 형식 파일 경로
...
{{% blocks/products/pf/agp/code-block title="SHX를 GEOJSON으로 저장 - C#" offSpacer="" %}}

```cs
// SHX GIS 데이터를 GEOJSON 데이터로 변환합니다.
...
{{< blocks/products/pf/agp/demobox sectionTitle="SHX를 GEOJSON으로 변환하는 라이브 데모" sectionDescription="저희 [라이브 데모 웹사이트](https://products.aspose.app/gis/conversion/shx-to-geojson)를 방문하여 SHX를 즉시 GEOJSON으로 변환하세요. 라이브 데모에는 다음과 같은 이점이 있습니다." >}}
        ...
        {{< blocks/products/pf/agp/democard icon="fa-file-text" text="SHX 파일을 업로드하면 즉시 GEOJSON으로 변환됩니다." >}}
...
특성 지오메트리의 위치 인덱스 형식으로, 특징 지오메트리를 빠르게 앞뒤로 검색할 수 있습니다.
자세히 보기
...
지리적 특징과 비 공간 속성을 나타내도록 설계된 JSON 기반 형식입니다. 이 형식을 통해 다양한 JSON (JavaScript Object Notation) 객체와 결합 방식이 정의됩니다. JSON 형식은 지리적 특징, 공간 범위 및 속성에 대한 집합 정보를 나타냅니다. 이 파일의 개체는 포인트, 라인 문자열, 폴리곤과 같은 기하 도형을 나타낼 수 있으며, 특징 또는 특징 모음으로도 사용할 수 있습니다. 특징은 주소와 장소를 점으로, 도로와 주요 도로를 선 문자열로, 국가, 지방 및 토지 지역을 폴리곤으로 반영합니다. GeoJSON을 사용하면 다양한 모바일 라우팅 및 탐색 애플리케이션에서 서비스 범위를 나타낼 수 있습니다. TopoJSON은 크기가 더 작고 지리 공간 위상 구조를 인코딩하는 GeoJSON의 확장입니다.
자세히 보기
